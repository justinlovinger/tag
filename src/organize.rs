...
fn to_move_ops(files: Files, prefix: Prefix) -> impl Iterator<Item = MoveOp> + '_ {
    ...
    untagged_files
        ...
        .chain(inline_tags.into_iter().map(move |(file, mut tags)| {
...
mod files {
    ...
    pub type TagsFiles<'a> = HashMap<&'a TagRef, HashSet<ByThinAddress<&'a TaggedFile>>>;
    ...
    impl<'a> Files<'a> {
        ...
        pub fn finalize(self) -> (UntaggedFiles<'a>, InlineTags<'a>) {
            (self.untagged_files, self.inline_tags)
        }
    }
    ...
    fn extract_inline_tags_into<'a>(
        tags_files: &mut TagsFiles<'a>,
        inline_tags: &mut InlineTags<'a>,
    ) {
